...
jQuery.getJSON(
    ...
    {format:"json"}).done(function (data) {
        var times = new Array();
        var ydata = new Array();
        // iterate over json data and convert temp and from string to date objects
        for (i in data) {
         ...
         // create ranges and scales for plotting
         var range_round = 1,
        ...
        vrules.append("svg:line")
            ...
            .attr("class", "axis")
            .attr("x1", x(date_start)).attr("y1", y(yrange_min))
            .attr("x2", x(date_start)).attr("y2", y(yrange_max));
        ...
        // horizontal grid lines
         hrules.append("svg:line")
             .attr("x1", x(date_start)).attr("y1", y)
             .attr("x2", x(date_end)).attr("y2", y);

        // horizontal axis line
        hrules.append("svg:line")
            .attr("class", "axis")
            .attr("x1", x(date_start)).attr("y1", y(yrange_min))
            .attr("x2", x(date_end)).attr("y2", y(yrange_min));
